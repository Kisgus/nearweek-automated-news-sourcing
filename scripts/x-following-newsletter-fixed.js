    ...
    for (let i = 0; i < maxAccounts; i += batchSize) {
      ...
      await Promise.all(batch.map(async (user) => {
        try {
          ...
          const userDetails = await this.v2Client.userByUsername(user.username, {
          ...
          const tweets = await this.v2Client.userTimeline(userDetails.data.id, {
            max_results: 10,
          ...
          if (tweets.data && tweets.data.length > 0) {
            ...
            if (recentTweets.length > 0) {
              const postsWithAuthor = recentTweets.map(tweet => ({
                ...
                relevance_score: this.calculateRelevance(tweet, userDetails.data),
                url: `https://x.com/${userDetails.data.username}/status/${tweet.id}`
              }));
